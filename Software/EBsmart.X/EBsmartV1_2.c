 ...
 * PID, offset, etc
 * 1.2 - Alteração salvar valor volume água inicial (subtrair offset), corrigido
 *      bug de transbordar central de agua se iniciar com a central já cheia,
 *      acionar saidas que estão sobrando para monitorar comunicação serial
 *      com o display DWIN
 *
 * A fazer:
...
#define RELEPANELA FOUT5
#define RELEPANELAON OUT5ON
#define RELEPANELAOFF OUT5OFF
#define RESISTENCIARESERVATORIO FOUT6
#define RESISTENCIARESERVATORIOON OUT6ON
#define RESISTENCIARESERVATORIOOFF OUT6OFF
...
#define VOLUMEASUBTRAIREEPROM 80                                                //subtrai 80 litros do valor no display DWIN para salvar na EEPROM
                                                                                //um valor que não ultrapasse 255
#define NUMTELAAGUARDAR 38
#define NUMTENTATIVASCOMUNDWIN 10
...
void EscreverDWINram(unsigned int endereco,int valor){
    ...
    OUT7ON;                                                                     //saida vazia, usada para monitorar comun serial
    ...
    OUT7OFF;                                                                    //saida vazia, usada para monitorar comun serial
...
int LerDWINram(unsigned int endereco)
{
    ...
    OUT6ON;                                                                     //saida vazia, usada para monitorar comun serial
    ...
    OUT6OFF;                                                                    //saida vazia, usada para monitorar comun serial
...
unsigned char LeDadosReceita(void)
{
    ...
    for(utemp8=0x10;utemp8<0x26;utemp8++){
        switch(utemp8){
            ...
            case 0x23: ReceitaAtual.VolumeInicial = LerDWINram(utemp8)-VOLUMEASUBTRAIREEPROM;break;
    ...
    //diametro da panela 87,5cm
    //87,5^2*PI/4 = 6013
    mmAguaPrimaria = (long)(ReceitaAtual.VolumeInicial+VOLUMEASUBTRAIREEPROM)*10000;
    mmAguaPrimaria /= 6013;
...
void ExecutaBrassagem(void)
{
    ...
    if( ((EstadoBrassagem >= MASHIN) && (EstadoBrassagem <= MASHOUT) && (EstadoBomba == CENTRAL)) || (EstadoBrassagem == SPARGINGMANUAL) || (EstadoBrassagem == WHIRLPOOL) || ((EstadoBrassagem >= PREFERVURA) && (EstadoBrassagem <= FINALIZARFERVURA)) ){
        ...
        switch(EstadoPrincipal){
            ...
            case EDICAORECEITA:
                ...
                if(LerDWINram(DWINadrBitsReceitaBrassagem) && MASKDOWNLOADRECEITA){ //pressionado icone para salvar receita
                    EscreverDWINreg(3,38);                                      //exibe tela aguardar
                    ...
                    EscreverDWINreg(3,4);                                       //volta tela de receita
                    EscreverDWINram(DWINadrBitsReceitaBrassagem,0);
                }
                if(LerDWINreg(03) == NUMTELAAGUARDAR){                          //saiu da edição da receita sem salvar
                    if(LeDadosReceita()){                                       //verifica se dados estão consistentes
                        ReceitaCarregada = 1;
                        EscreverDWINreg(3,1);                                       //exibe menu principal
                        EstadoPrincipal = MENUPRINCIPAL;
            ...
            case CONFIGURACAO:
                if(LerDWINreg(03) == NUMTELAAGUARDAR){                          //voltou para menu principal
